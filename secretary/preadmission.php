...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && empty($error)) {
    ...
    // ---------------- Insertion admission ----------------
    $selected_medecin = $_POST['personnel_id'] ?? '';
    $service_id = $_POST['service_id'] ?? null;
    $personnel_name = null;

    if ($selected_medecin) {
        $stmt = $pdo->prepare("SELECT personnel_name FROM ap_personnels WHERE personnel_id = ?");
        $stmt->execute([$selected_medecin]);
        $personnel_name = $stmt->fetchColumn();
    }
    ...
    if (!$error && $patient_social && $admission_type && $hospitalisation_date) {
        try {
            ...
            (admission_type, hospitalisation_date, intervention_time, private_room, reason, notes, statut, personnel_id, personnel_name, service_id, patient_social, chambre_id)
            VALUES (:t, :hd, :it, :pr, :r, :n, 'pré-admission', :p, :pn, :srv, :s, :c)
            ");
        ...
        <form method="POST" id="admissionForm" enctype="multipart/form-data" onsubmit="return validateForm()">
            ...
            <div class="form-step" id="step2">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        <label class="form-label required">Médecin responsable</label>
                        <select name="personnel_id" class="form-select" id="medecinSelect">
                            <option value="">Sélectionner un médecin</option>
                            <?php foreach($medecins as $m): ?>
                                <option 
                                    value="<?= htmlspecialchars($m['personnel_id']) ?>"
                                    data-service="<?= htmlspecialchars($m['service_id']) ?>"
                                >
                                    <?= htmlspecialchars($m['personnel_name']) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                        <!-- champ caché pour service id -->
            ...
            <a href="secretary_dashboard.php" class="btn btn-secondary">
                ⬅ Dashboard
            </a>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        Voulez-vous générer la fiche PDF de cette pré-admission ?
      </div>
      ...
      <div class="modal-footer">
        ...
        <a href="#" id="confirmPdfBtn" class="btn btn-danger">
          Oui, générer
        </a>
      </div>
