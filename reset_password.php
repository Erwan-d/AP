...
if (isset($_GET['token'])) {
    ...
    $stmt = $pdo->prepare("SELECT personnel_id FROM personnels WHERE reset_token = ? AND reset_expires > NOW()");
    $stmt->execute([$token]);
    ...
    if ($user) {
        if ($_SERVER["REQUEST_METHOD"] === "POST") {
            ...
            if ($password === $confirm && strlen($password) >= 6) {
                ...
                $update = $pdo->prepare("UPDATE personnels SET password = ?, reset_token = NULL, reset_expires = NULL WHERE personnel_id = ?");
